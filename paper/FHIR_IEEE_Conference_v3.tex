...
%   fig_login.png          (verificación segundo factor / autenticación)
%   fig_formulario.png     (formulario admisión paciente)
%   fig_observation.png    (respuesta JSON endpoint FHIR)
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_arquitectura.png}}
\caption{Arquitectura distribuida del prototipo. Tres nodos hospitalarios
independientes interconectados mediante VPN site-to-site, cada uno con
firewall perimetral (ASA), middleware, router y bases de datos propias.
La capa superior integra servidor FHIR, balanceador de carga (LOAD),
gateway API y acceso HTTPS para usuarios. Fuente: prototipo historia
cl\'inica distribuida FHIR (may-2026).}
\label{fig:arq}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_formulario.png}}
\caption{M\'odulo de admisi\'on del prototipo. El formulario captura
datos cl\'inicos del paciente (identificaci\'on, datos demogr\'aficos,
informaci\'on de contacto y aseguradora) que son transformados en
un recurso FHIR \texttt{Patient} y persistidos en el nodo activo.
Fuente: prototipo historia cl\'inica distribuida FHIR (may-2026).}
\label{fig:formulario}
\end{figure}

La seguridad fue implementada con autenticaci\'on de doble factor
y tokens JWT (Fig.~\ref{fig:login}): (1) el usuario inicia sesi\'on;
(2) el sistema genera y env\'ia un c\'odigo de verificaci\'on de
segundo factor; (3) el frontend obtiene el token JWT firmado;
(4) el token es validado por el servidor HAPI FHIR antes de
procesar cada solicitud cl\'inica.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.7\columnwidth]{fig_login.png}}
\caption{M\'odulo de verificaci\'on de segundo factor del sistema.
Implementa autenticaci\'on multifactor (MFA) para garantizar el
acceso seguro a los nodos FHIR cl\'inicos.
Fuente: prototipo historia cl\'inica distribuida FHIR (may-2026).}
\label{fig:login}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{fig_observation.png}}
